...
import std.stdio : File, writef, writefln;
...
nothrow @trusted:
    ...
    public:
        ...
        void setColor(Color color)
        {
            ...
            attr = (attr & ~(FOREGROUND_WHITE | FOREGROUND_INTENSITY)) | ((color & Color.red)
                    ? FOREGROUND_RED : 0) | ((color & Color.green) ? FOREGROUND_GREEN
                    : 0) | ((color & Color.blue) ? FOREGROUND_BLUE
                    : 0) | ((color & Color.bright) ? FOREGROUND_INTENSITY : 0);
    ...
    static Console* create(ref File fp)
    {
        return Console.create(assumeWontThrow(fp.getFP()));
    }
...
char getch()
{
    import core.sys.posix.termios : tcgetattr, tcsetattr, ICANON, termios, VMIN, VTIME, TCSANOW, ECHO;
    import core.sys.posix.unistd : read;
    import core.sys.posix.fcntl : open, O_RDWR;
    immutable fd = open("/dev/tty", O_RDWR);
    if (fd < 0) {
        return 0;
    }
